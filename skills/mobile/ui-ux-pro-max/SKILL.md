---
name: ui-ux-pro-max
description: "UI/UX design intelligence for web and mobile. Searchable local database with 84 styles, 192 color palettes, 74 font pairings, 192 product types, 98 UX guidelines, 104 icon entries, 16 GSAP motion presets, and 25 chart types across 22 stacks (React, Next.js, Vue, Nuxt, Svelte, Astro, SwiftUI, React Native, Flutter, Tailwind, shadcn/ui, Jetpack Compose, Angular, Laravel, JavaFX, WPF, WinUI, Avalonia, Uno Platform, UWP, Three.js, and HTML/CSS). Use when designing, building, or reviewing UI: pages, components, color schemes, typography, layout, accessibility, animation, or data visualization. For React Native specifically, this is a general aesthetic/color/typography database — prefer react-native-design first for native structural and platform-feel decisions, then use this skill to fill in color palettes, font pairings, and style direction."
license: MIT
metadata:
  area: mobile
  upstream:
    ...
    note: "Adapted: replaced Claude Code's ${CLAUDE_PLUGIN_ROOT} env var references with a resolvable ${UI_UX_PRO_MAX_SKILL_DIR} placeholder + resolution instructions, since Cursor has no equivalent env var. Also added a cross-reference to react-native-design for React Native structural/native-feel decisions (this skill covers general aesthetics — colors, typography, styles — not platform-native structure)."
---
...
`${UI_UX_PRO_MAX_SKILL_DIR}` below is **not** a real environment variable (that's a Claude Code plugin convention this skill was adapted from) — it stands for the absolute local path to this skill's own folder. Resolve it yourself before running any command: if this repo is checked out locally, it's `<path-to-skills-repo>/skills/mobile/ui-ux-pro-max`; if installed as a Cursor plugin, check `~/.cursor/plugins/*/skills/mobile/ui-ux-pro-max`.

```bash
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "<query>" --domain <domain>
```

If `python` is not found, try `python3`, then `py -3` (Windows). Requires Python 3.x, no external dependencies.
...
```bash
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "<query>" --design-system --persist -p "Project Name" --output-dir "<project-root>"
```
...
```bash
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```

### Step 3: Supplement with Detailed Searches (as needed)

```bash
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "<keyword>" --stack <stack>
```
...
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "AI search tool modern minimal" --design-system -p "AI Search"

# Step 3: supplement
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "search loading animation" --domain ux

# Step 4: stack guidelines
python "${UI_UX_PRO_MAX_SKILL_DIR}/scripts/search.py" "suspense streaming bundle" --stack nextjs
```
