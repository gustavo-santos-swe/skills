---
name: diagnose
description: Use when something is broken, flaky, regressing, or failing tests — before proposing a fix. Forces evidence-first debugging to root cause, then fix and lock it down.
...
- Don't propose a fix before repro (simple) or before a red loop (hard).
- Don't "fix" three things at once to see what sticks.
- Don't treat "works on my machine once" as done without the lock or original repro green.
- Don't open tracker tickets here — this skill fixes (or proves it can't yet).
- Don't paste other debugging playbooks; this file is the source of truth.
