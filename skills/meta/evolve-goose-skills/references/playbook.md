...
6. **Craft + voice.** `writing-great-skills` for predictability; `write-like-goose` for durable prose and grill questions (answer first, then teach the why).
...
| Concern | Owner |
|---------|--------|
| House voice | `write-like-goose`: answer first, then teach the why; shape from the job (+ AGENTS / Cursor rule). Owns Question surface wording. |
...
| Session lessons → absorb / local product skill / hand off | `skill-retro` |
