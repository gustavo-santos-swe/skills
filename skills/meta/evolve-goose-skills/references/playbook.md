...
6. **Craft + voice.** `writing-great-skills` for predictability; `write-like-goose` for durable prose and grill questions (answer-first prose, fully answers).
...
| Concern | Owner |
|---------|--------|
| House voice | `write-like-goose`: answer-first prose, fully answers (+ AGENTS / Cursor rule). Owns Question surface wording. |
