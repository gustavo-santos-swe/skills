...
Voice: **`write-like-goose`**. Grill wording → that skill's **Question surface**.  
...
4. **Questionnaire** - grill **one decision at a time** per [`questionnaire.md`](references/questionnaire.md). Word each ask per **`write-like-goose` Question surface**. Recommend + why; wait. Cover purpose, non-goals, ownership, branches, invocation.
...
3. One question at a time in questionnaires, worded so the user can answer it (`write-like-goose` Question surface); recommend first.
